  ...
  _createClass(HtmlWebpackReplaceHost, [{
    ...
    value: function _setAttributesSrc(tag, replaceStr, version) {
      ...
      if (process.env.NODE_ENV == 'daily' || process.env.NODE_ENV == 'local') {
        ...
        var _src = replaceStr ? tag.attributes.src.replace(regex, replaceStr) : tag.attributes.src.replace(regex, './').replace(/src\/[\w/]*\//, ''); // if (tag.attributes.src.match(/javascripts\/build\//)) {
